  ...
  $.complety.Complety = (function () {
    ...
    Complety.prototype = {
      ...
      _highlight: function (suggestion) {
        ...
        var value = this._cleanRegExp(this.el.val());
      ...
      _highlightItem: function (number) {
        ...
        this.target.data('current', index).find(this.opt.wrappers.item).removeClass(clazz).eq(index).addClass(clazz);
      },
      ...
      _isIgnoredKey: function (key) {
        return [keys.ALT, keys.COMMAND, keys.CTRL, keys.LEFT, keys.RIGHT, keys.SHIFT].indexOf(key) >= 0;
      },
      ...
      _query: function () {
        if (this._able()) {
          var url = this._cleanRegExp(this.url());
          var cached = this.opt.cache && this.opt.cacheData[url];

          if (cached) {
            this._always(cached);
            this._done(cached);
          } else {
            var ajax = this._ajax(this._extractParams(), url);
